	...
	'mailMessages': function (data) {                                                             // 4
		...
		if (data.language !== 'en') {                                                                // 55
			...
			if (localeFn) {                                                                             // 57
				...
				moment.locale(data.language);                                                              // 59
			}                                                                                           // 60
		}                                                                                            // 61
                                                                                               //
		...
		var html = RocketChat.models.Messages.findByRoomIdAndMessageIds(data.rid, data.messages, {   // 65
			sort: {                                                                                     // 66
				ts: 1                                                                                      // 66
			}                                                                                           // 66
		}).map(function (message) {                                                                  // 65
			var dateTime = moment(message.ts).locale(data.language).format('L LT');                     // 68
		...
		Meteor.defer(function () {                                                                   // 72
			Email.send({                                                                                // 73
				to: emails,                                                                                // 74
				from: RocketChat.settings.get('From_Email'),                                               // 75
				replyTo: email,                                                                            // 76
				subject: data.subject,                                                                     // 77
				html: header + html + footer                                                               // 78
			});                                                                                         // 73
			return console.log("Sending email to " + emails.join(', '));                                // 80
		});                                                                                          // 81
		return {                                                                                     // 82
			success: true,                                                                              // 83
			missing: missing                                                                            // 84
