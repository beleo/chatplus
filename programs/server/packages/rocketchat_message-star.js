...
Meteor.methods({                                                                                       // 1
	starMessage: function (message) {                                                                     // 2
		...
		if (!RocketChat.settings.get('Message_AllowStarring')) {                                             // 9
			throw new Meteor.Error('error-action-not-allowed', 'Message starring not allowed', {                // 10
				method: 'pinMessage',                                                                              // 11
				action: 'Message_starring'                                                                         // 12
			});                                                                                                 // 10
		}                                                                                                    // 14
                                                                                                       //
		var room = RocketChat.models.Rooms.findOneById(message.rid);                                         // 16
                                                                                                       //
		if (Array.isArray(room.usernames) && room.usernames.indexOf(Meteor.user().username) === -1) {        // 17
			return false;                                                                                       // 18
		}                                                                                                    // 19
                                                                                                       //
