...
var reg = /--(rc-color-.*?): (.*?);/igm;                                                                       // 14
var colors = [].concat((0, _toConsumableArray3.default)(Assets.getText('client/imports/general/variables.css').match(reg))).map(function (color) {
	var _color$split = color.split(': '),                                                                         // 16
	    _color$split2 = (0, _slicedToArray3.default)(_color$split, 2),                                            // 16
	    name = _color$split2[0],                                                                                  // 16
	    value = _color$split2[1];                                                                                 // 16
                                                                                                               //
	return [name.replace('--', ''), value.replace(';', '')];                                                      // 18
});                                                                                                            // 19
colors.forEach(function (_ref) {                                                                               // 21
	var _ref2 = (0, _slicedToArray3.default)(_ref, 2),                                                            // 21
	    key = _ref2[0],                                                                                           // 21
	    color = _ref2[1];                                                                                         // 21
                                                                                                               //
	if (/var/.test(color)) {                                                                                      // 22
		var _color$match = color.match(/var\(--(.*?)\)/i),                                                           // 22
		    _color$match2 = (0, _slicedToArray3.default)(_color$match, 2),                                           // 22
		    value = _color$match2[1];                                                                                // 22
                                                                                                               //
		return RocketChat.theme.addPublicColor(key, value, 'Colors', 'expression');                                  // 24
	}                                                                                                             // 25
                                                                                                               //
	RocketChat.theme.addPublicColor(key, color, 'Colors');                                                        // 26
});                                                                                                            // 27
var majorColors = {                                                                                            // 29
	'content-background-color': '#FFFFFF',                                                                        // 30
	'primary-background-color': '#04436A',                                                                        // 31
	'primary-font-color': '#444444',                                                                              // 32
	'primary-action-color': '#13679A',                                                                            // 33
	// was action-buttons-color                                                                                   // 33
	'secondary-background-color': '#F4F4F4',                                                                      // 34
	'secondary-font-color': '#A0A0A0',                                                                            // 35
	'secondary-action-color': '#DDDDDD',                                                                          // 36
	'component-color': '#EAEAEA',                                                                                 // 37
	'success-color': '#4dff4d',                                                                                   // 38
	'pending-color': '#FCB316',                                                                                   // 39
	'error-color': '#BC2031',                                                                                     // 40
	'selection-color': '#02ACEC',                                                                                 // 41
	'attention-color': '#9C27B0'                                                                                  // 42
}; // Minor colours implement major colours by default, but can be overruled                                   // 29
                                                                                                               //
var minorColors = {                                                                                            // 46
	'tertiary-background-color': '@component-color',                                                              // 47
	'tertiary-font-color': '@transparent-lightest',                                                               // 48
	'link-font-color': '@primary-action-color',                                                                   // 49
	'info-font-color': '@secondary-font-color',                                                                   // 50
	'custom-scrollbar-color': '@transparent-darker',                                                              // 51
	'status-online': '@success-color',                                                                            // 52
	'status-away': '@pending-color',                                                                              // 53
	'status-busy': '@error-color',                                                                                // 54
	'status-offline': '@transparent-darker'                                                                       // 55
}; // Bulk-add settings for color scheme                                                                       // 46
                                                                                                               //
Object.keys(majorColors).forEach(function (key) {                                                              // 59
	var value = majorColors[key];                                                                                 // 60
	RocketChat.theme.addPublicColor(key, value, 'Old Colors');                                                    // 61
});                                                                                                            // 62
Object.keys(minorColors).forEach(function (key) {                                                              // 64
	var value = minorColors[key];                                                                                 // 65
	RocketChat.theme.addPublicColor(key, value, 'Old Colors (minor)', 'expression');                              // 66
});                                                                                                            // 67
...
RocketChat.settings.add('theme-custom-css', '', {                                                              // 71
	group: 'Layout',                                                                                              // 72
	type: 'code',                                                                                                 // 73
	code: 'text/css',                                                                                             // 74
	multiline: true,                                                                                              // 75
	section: 'Custom CSS',                                                                                        // 76
	"public": true                                                                                                // 77
});                                                                                                            // 71
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
